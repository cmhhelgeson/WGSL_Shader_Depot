...
fn Morton2d_16x16(t: u32) -> u32     // --------EAFBGCHD
{
...
fn SplatIndexToPixelIndex(idx: u32) -> vec3<u32> {
  var res: vec3<u32>;
  var xy: var2<u32> = Morton2D_16x16(idx);
  var widht: u32 = 2048 / 16;
  idx >> 8;
...
fn LoadSplatPosTex(
  ...
  //var val: vec3<f32> = _TexPos.Load(coord).rgb;
  if (tex_format & 1) {
...
@compute
//SV_DispatchThreadID roughly equivlanet to WGSL's global_invocation_id
fn computeMain(
  ...
  var coord = SplatIndexToPixelIndex(originalIndex);
